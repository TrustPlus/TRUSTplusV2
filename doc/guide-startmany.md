...
[From the trustplus github repo](https://github.com/trustpluscoin/trustpluscoin/blob/master/doc/masternode_conf.md)
...
If you are using a `masternode.conf` file you no longer need the `trustplus.conf` file. The exception is if you need custom settings (_thanks oblox_). In that case you **must** remove `masternode=1` from local `trustplus.conf` file. This option should be used only to start local Hot masternode now.

## Update trustplus.conf on server

If you generated a new masternode private key, you will need to update the remote `trustplus.conf` files.

Shut down the daemon and then edit the file.

```nano .trustpluscore/trustplus.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `trustplus.conf` file.
...
Search your Masternodes on https://trustplusninja.pl/masternodes.html
